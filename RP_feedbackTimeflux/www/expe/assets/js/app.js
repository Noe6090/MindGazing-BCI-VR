...
function componentToHex(component) {
    const hex = component.toString(16); // Convert to hex
    return hex.length == 1 ? "0" + hex : hex; // Add leading zero if necessary
}



function highlight_best_target(data, mappingFunction, max_width, momentum_floor, momentum_ceil, min_width = 3) {
    
    // Remove the 'best-target' class from all targets
    const allTargets = document.querySelectorAll('#keyboard .target');
    allTargets.forEach(target => {
        target.classList.remove('best-target');
    });

    // Take the last set of correlation values
    const lastEntry = Object.values(data)[Object.values(data).length - 1];
    const correlationValues = Object.values(lastEntry);

    // Compute max value and find the index of the best target before normalization
    let max_value = Math.max(...correlationValues);
    let bestTargetIndex = correlationValues.indexOf(max_value);

    // Normalize the max_value with momentum range 
    max_value = (max_value - momentum_floor) / (momentum_ceil - momentum_floor);

    // Proceed only if max_value is greater than 0
    if (max_value > 0.0) {
        // Use mapping function to determine color saturation and border width proportion
        let saturation = mappingFunction(max_value);

        // COLOR COMPUTATION - Generate the border color based on the saturation
        let colorValue = Math.floor(saturation * 255); 
        let borderColor = rgbToHex(colorValue, Math.floor(colorValue / 2), 0); // Orange

        // BORDER WIDTH COMPUTATION - Scale from min_width to max_width
        let borderWidthInt = Math.floor(min_width + saturation * (max_width - min_width));
        let borderWidthPx = `${borderWidthInt}px`;

        // Highlight the target with the best correlation
        const bestTargetElement = allTargets[bestTargetIndex]; 
        if (bestTargetElement) {
            // Check if the element doesn't already have the "success" or "failure" class
            if (!bestTargetElement.classList.contains('success') && !bestTargetElement.classList.contains('failure')) {
                // Apply computed border color and width through CSS variables
                set_css_var('--best-target-border-color', borderColor);
                set_css_var('--best-target-border-width', borderWidthPx);
                bestTargetElement.classList.add('best-target'); // Highlight the best target
            } else {
                console.warn('Best target element has "success" or "failure" class, skipping highlight.');
            }
        } else {
            console.warn('Best target element not found.');
        }
    }
}


function xPow(x, pow = 2) {
    return Math.pow(x , pow);
}

function beta_s(x, beta = 3) {
    return 1 / (1 + Math.pow(x / (1 - x), -beta));
}
...
class Burst {
    ...
    async task_cue() {
        ...
        console.log(`Cuing sequence ${this.options.task.cue.targets}`)
        // Cue selected targets and wait for a prediction
        for (let index of this.options.task.cue.targets) {
            console.log(`Current index to be cued ${index}`)
            ...
            if (this.options.task.cue.enable_feedback) {
...
load_settings().then(async settings => {
    ...
    // Highlight of targets
    if (burst.options.feedback.enable_feedback){ 
        burst.io.subscribe('correlations');
        console.log('Subscribed to correlations event');

        burst.io.on('correlations', (data, meta) => {
            highlight_best_target(data,xPow,burst.options.feedback.max_width,burst.options.task.values.min / 2,burst.options.task.values.max);
        });
    }
    ...
    // Start free run
    stages[4] = async () => {
        if (burst.options.training.enable) {
        notify(
            'Training session',
    ...
    stages[6] = async () => {
        ...
        || burst.options.layout.task == 'grid'
            ...
            notify(
                'Pinpad session',
                `Now, the task starts.<br> You will see codes on the bottom left of the screen. <br> Your goal is to activate the targets to type the sequences that will be displayed.<br> If you typed right, the button will be colored in green. Otherwise it will be red.<br> The session is made of ${burst.options.task.sequence.sequences.length} sequences of ${burst.options.task.sequence.sequences[0].length} digits.`,
                'Press any key to continue'
            )
            await interaction();
            toggle('overlay');
