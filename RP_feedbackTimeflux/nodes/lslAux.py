    ...
    def update(self):
        ...
        if self._inlet:
            ...
            if stamps: 
                ...
                # Get the base time from Timeflux's clock
                base_time = np.datetime64(now(), "us")  # Asegurar que base_time sea datetime64[us]

                # Calculate relative differences between LSL timestamps
                relative_offsets = np.array(stamps) - stamps[0] # Relative differences in seconds
                relative_offsets = (relative_offsets * 1e6).astype("timedelta64[us]")  # Convert to microsecs

                # Adjust relative timestamps to the base time
                adjusted_stamps = base_time + relative_offsets

                # Convert adjusted timestamps to pandas datetime format
                adjusted_stamps = pd.to_datetime(adjusted_stamps)


                # Assign the data and adjusted timestamps to the output
